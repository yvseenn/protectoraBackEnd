...
async function register(mail,pwd,name){
    await axios.post(BASEURL + "/register", {email:mail, password:pwd, name: name})
}
...
return (
    <PostContext.Provider value={{posts:state.posts, user:state.user, individualPost:state.individualPost,addPost,login,register,changeName,closeSession,getIndividualPost,deletePost}}>
        {children}
